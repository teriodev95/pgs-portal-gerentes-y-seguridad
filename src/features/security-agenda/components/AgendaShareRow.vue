...
<template>
  <div class="rounded-lg border border-gray-200 bg-white p-3">
    <div class="flex items-center gap-2">
      ...
      <!-- Botones de texto, pero con alto tocable de 44px -->
      <div class="flex items-center gap-1 text-sm font-medium text-blue-700">
        <button type="button" class="min-h-[44px] px-2" :disabled="busy" @click="emit('share')">
          Compartir
        </button>
        <button type="button" class="min-h-[44px] px-2" :disabled="busy" @click="emit('copy')">
          Copiar
        </button>
        <button
          type="button"
    ...
    <div v-if="confirming" class="mt-3 border-t border-gray-100 pt-3">
      ...
      <div class="mt-2 flex gap-1 text-sm font-medium">
        <button type="button" class="min-h-[44px] px-2 text-red-700" @click="revoke">
          Sí, revocar
        </button>
        <button type="button" class="min-h-[44px] px-2 text-gray-700" @click="confirming = false">
          Cancelar
        </button>
      </div>
    </div>
